...
// Mantine
import { Modal, Text, Group, Badge, Stack, Title } from "@mantine/core"
...
export function UserTable({ users, onEdit, onDelete, onArchive }: UserTableProps) {
  ...
  // Archive / Unarchive click
  const handleArchiveClick = (user: User) => {
    setUserToArchive(user)
    setArchiveMode(user.isArchived ? "unarchive" : "archive")
  ...
  const handleArchiveConfirm = async () => {
    ...
    try {
      ...
      const nextArchived = archiveMode === "archive"
      await onArchive(userToArchive._id, nextArchived)
    ...
    <>
                    ...
                    <div className="flex items-center justify-end">
                      ...
                      <div className="hidden sm:flex gap-2">
                        ...
                        <Button
                          ...
                          size="icon"
                          className={`h-8 w-8 cursor-pointer border-[0px] ${
                            isArchived ? "text-amber-600" : ""
                          }`}
                          onClick={() => handleArchiveClick(user)}
                          title={isArchived ? "Unarchive user" : "Archive user"}
                        >
      ...
      >
        {viewUser && (
          <Stack
            gap="sm"
            className="border-[1px] border-gray-500 px-4 py-5 rounded-sm"
          >
            ...
            <DialogDescription>
              {archiveMode === "archive" ? (
                <>
                  ...
                  and hide them from active user lists. You can unarchive them
                  later.
                </>
              ) : (
                <>
                  ...
                  {userToArchive &&
                    ` ${userToArchive.firstName} ${userToArchive.lastName}`}{" "}
                  and show them in active user lists again.
                </>
              )}
            </DialogDescription>
          </DialogHeader>
...
//   const handleArchiveClick = (user: User) => {
//     setUserToArchive(user)
//     setArchiveMode(user.isArchived ? "unarchive" : "archive") // decide mode based on current state
//     setOpenArchiveUser(true)
//   }
...
//       const nextArchived = archiveMode === "archive" // true = archive, false = unarchive
//       await onArchive(userToArchive._id, nextArchived)
...
//       {/* ... delete dialog & view modal unchanged except status badge you already have ... */}

//       {/* Archive / Unarchive user modal */}
//       <Dialog open={openArchiveUser} onOpenChange={setOpenArchiveUser}>
//         <DialogContent
//           onOpenAutoFocus={(e) => e.preventDefault()}
//           className="bg-white dark:bg-darkModal"
